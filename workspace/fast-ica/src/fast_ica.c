...
void gram_schmidt_decorrelation(Matrix *w_i_new, Matrix *w, int i) {
    if (i > 0) {
        ...
        Matrix *tmp_mat = mat_mul_t1(w_slice, w_slice);
        free_mat(w_slice);
...
void fast_ica(FastICAArgs *arg) {
    ...
    switch (arg->strategy) {
        case Parallel:
            ...
            w = ica_par(x_w, g, arg->threshold, arg->max_iter);
            break;
        case Deflation:
            // Execute deflation algorithm
            w = ica_def(x_w, g, arg->threshold, arg->max_iter);
            break;
        default:
