...
Matrix *sine_mat(Matrix *m) {
    Matrix *s = new_mat(m->height, m->width, false);
...
Matrix *generate_sine_wave(fp amp, fp freq, fp phase, int n_samples, fp range) {
    ...
    Matrix *t = linspace(0, range, n_samples, false);
...
Matrix *generate_signals(int n_samples, fp range, bool add_noise) {
    ...
    // Stack them
    Matrix *s = new_mat(3, n_samples, false);
    paste_row(s, s1, 0);
    paste_row(s, s2, 1);
    paste_row(s, s3, 2);
    ...
    if (add_noise) {
        Matrix *ns = mat_randn(s->height, s->width, false);
        scale_(ns, 0.2f);
        add_mat_(s, ns);
        free_mat(ns);
    }
