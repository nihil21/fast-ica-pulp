...
Matrix *to_hessenberg(Matrix *m) {
    ...
    for (int k = 0; k < n - 2; k++) {
        ...
        Matrix *tmp1 = mat_mul_t1(v_k, h_k);
...
Matrix *covariance(Matrix *x, bool center_data) {
    ...
    if (center_data) {  // center data first
        ...
        x_cov = mat_mul_t2(x_c, x_c);  // transpose handled internally for efficiency reasons
        ...
        // Compute covariance matrix
        x_cov = mat_mul_t2(x, x);  // transpose handled internally for efficiency reasons
    }
